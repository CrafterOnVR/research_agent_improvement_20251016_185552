...
class AdvancedBrowserController:
    ...
    def switch_to_iframe(self, selector: str, by: str = "css", timeout: int = 10) -> bool:
        """Switches driver context to an iframe."""
        try:
            iframe_element = self.find_element_advanced(selector, by, timeout)
            if not iframe_element:
                logging.warning(f"Iframe not found with selector: {selector}")
                return False
            self.driver.switch_to.frame(iframe_element)
            logging.info(f"Switched to iframe with selector: {selector}")
            return True
        except Exception as e:
    ...
    def switch_to_default_content(self):
        """Switches driver context back to the main document."""
        try:
