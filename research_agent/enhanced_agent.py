...
class EnhancedResearchAgent(ResearchAgent):
    ...
    def _initialize_advanced_controllers(self, safety_config: Optional[str] = None):
        ...
        try:
            ...
            self.memory_manager = MemoryManager(root=self.data_dir)
    ...
    def comprehensive_research(self, topic: str, research_config: Dict[str, Any] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            # Phase 6: Memorize Findings
            if self.memory_manager:
                print(f"Phase 6: Memorizing findings for '{topic}'")
                try:
                    doc_id = f"report-{results['topic']}-{results['timestamp']}"
                    metadata = {
                        "source": "comprehensive_research",
                        "topic": results['topic'],
                    ...
                    results["memorization_status"] = "success"
                except Exception as e:
                    logging.error(f"Failed to memorize findings: {e}")
                    results["memorization_status"] = f"failed: {e}"
